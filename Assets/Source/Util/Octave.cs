...
public class Octave {  // Simplex noise in 2D, 3D and 4D
  ...
  private static Vector4[] grad4 = new Vector4[]{new Vector4(0,1,1,1),new Vector4(0,1,1,-1),new Vector4(0,1,-1,1),new Vector4(0,1,-1,-1),
  ...
  public Octave(int seed) {
    ...
    for(int i=0;i<NUMBEROFSWAPS;i++){
        int swapFrom=rand2.Next(p.Length);
        int swapTo=rand2.Next(p.Length);
  ...
  private static double dot(Vector4 g, double x, double y) {
    return g.x*x + g.y*y; }

  private static double dot(Vector4 g, double x, double y, double z) {
